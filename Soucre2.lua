...
function Library:CreateWindow(Setting)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = tostring(TitleNameMain or "Duy Trường Hub - Blox Kid")
...
local Window = Library:CreateWindow({
    Title = "Duy Trường Hub - Blox Fruit",
    SubTitle = "",
    SaveFolder = "Hub.json",
...
task.delay(2, function()
	...
	Library:Notify({Title = "Duy Trường Hub", Content = "Script carregado! Bem-vindo, " .. game.Players.LocalPlayer.Name, Icon = "star", Duration = 5});
end);

InfoTab:AddSection(" Duy Trường ");
InfoTab:AddParagraph({
	Title = "Duy Trường Hub",
	Desc = "Version: V1 | Game: Blox Kid"
});
InfoTab:AddParagraph({
...
InfoTab:AddButton({
	...
	Callback = function()
		setclipboard("");
		Library:Notify({Title = "Duy Trường", Content = "", Icon = "bell", Duration = 5});
	end
});
InfoTab:AddParagraph({
	Title = " Credits",
	Desc = "Dev: Duy Trường Hub V1 - Blox Kid"
});
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Duy Trường Hub", Content = "Item especial encontrado! Auto Chest parado.", Icon = "bell", Duration = 6});
				return;
			end;
			...
			for _, entry in ipairs(chests) do
				...
				if _isSpecialChestItem() then
					_chestTweenActive = false;
					_G.Settings.Farm["Auto Farm Chest Tween"] = false;
					Library:Notify({Title = "Duy Trường Hub", Content = "Item especial encontrado!", Icon = "bell", Duration = 6});
					break;
				end;
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Duy Trường Hub", Content = "Chest Hop: limite atingido, trocando server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Duy Trường Hub", Content = "Item especial encontrado! Chest Bypass parado.", Icon = "bell", Duration = 6});
				return;
			end;
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Duy Trường Hub", Content = "Chest Hop: trocando server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
local function ShowResetConfirm()
	pcall(function()
		...
		simBtn.MouseButton1Click:Connect(function()
			...
			Library:Notify({Title = "Duy Trường Hub", Content = "Settings resetados! Reexecute o script.", Icon = "bell", Duration = 6});
		end);
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Duy Trường Hub", Content = "Estilo nao encontrado: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Duy Trường Hub", Content = "Compra enviada: " .. styleName, Icon = "check", Duration = 4});
end;
					...
					if PortalIslands and PortalIslands[_G.SelectIsland] then
						...
						if target then
							...
							if hrp and (hrp.Position - destCF.p).Magnitude < 20 then
								-- Chegou! Para o loop
								_G.BypassTeleportActive = false;
								Library:Notify({Title = "Duy Trường Hub", Content = "Chegou em: " .. _G.SelectIsland, Icon = "map-pin", Duration = 4});
							end;
						end;
					end;
				end);
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Duy Trường Hub", Content = "Quest completa!", Icon = "bell", Duration = 6});
						end);
