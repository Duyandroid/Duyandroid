...
-- _G.__userKey = nil
print("✅ Security verification passed. Loading Android hud...")
-- FIX XÁC ĐỊNH SEA CHO CẢ SERVER THƯỜNG VÀ VIP
local function detectSea()
    local placeId = game.PlaceId
    
    -- Kiểm tra PlaceId trực tiếp
    if placeId == 2753915549 then
        World1 = true; World2 = false; World3 = false
        return
    elseif placeId == 4442272183 then
        World1 = false; World2 = true; World3 = false
        return
    elseif placeId == 7449423635 then
        World1 = false; World2 = false; World3 = true
        return
    end
    
    -- DỰ PHÒNG: Kiểm tra bằng tên map trong game
    local workspace = game:GetService("Workspace")
    
    -- Kiểm tra Sea 1 (các map đặc trưng)
    if workspace:FindFirstChild("Jungle") or 
       workspace:FindFirstChild("Desert") or 
       workspace:FindFirstChild("Snow") or
       workspace:FindFirstChild("Marine") or
       workspace:FindFirstChild("Pirate Village") or
       workspace:FindFirstChild("Magma") then
        World1 = true; World2 = false; World3 = false
        return
    end
    
    -- Kiểm tra Sea 2
    if workspace:FindFirstChild("Dressrosa") or 
       workspace:FindFirstChild("GreenZone") or
       workspace:FindFirstChild("Colosseum") or
       workspace:FindFirstChild("Punk Hazard") or
       workspace:FindFirstChild("Zombie") then
        World1 = false; World2 = true; World3 = false
        return
    end
    
    -- Kiểm tra Sea 3
    if workspace:FindFirstChild("Zou") or 
       workspace:FindFirstChild("Hydra") or
       workspace:FindFirstChild("Turtle") or
       workspace:FindFirstChild("Mansion") or
       workspace:FindFirstChild("Castle") then
        World1 = false; World2 = false; World3 = true
        return
    end
    
    -- DỰ PHÒNG CUỐI: Kiểm tra bằng level của người chơi
    ...
    if success and level then
        ...
        -- Mặc định là Sea 1 nếu không xác định được
        World1 = true; World2 = false; World3 = false
    end
end
...
-- Tự động phát hiện lại khi teleport
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("Android hud") then
			makefolder("Android hud");
		end;
		if not isfolder("Android hud/Blox Fruits/") then
			makefolder("Android hud/Blox Fruits/");
		end;
		if not isfile(("Android Hud/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("Android hud/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("Android hud/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
	...
	if Update:SaveSettings() then
		if readfile and writefile and isfile and isfolder then
			if not isfile(("Android hud/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
				...
				writefile("Android hud/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
			end;
		else
			return warn("Status : Undetected Executor");
		end;
		...
		pcall(function()
			while wait(0.2) do
				...
				if game.Workspace._WorldOrigin.Locations:FindFirstChild("Frozen Dimension") then
					Frozencheck:Set(" Frozen Dimension Spawning");
					_G.FrozenSpawn = true;
				else
					Frozencheck:Set(" Frozen Dimension Not Spawn");
		...
		pcall(function()
			while wait(0.2) do
				if game.Workspace._WorldOrigin.Locations:FindFirstChild("Mirage Island") then
					Mirragecheck:Set("Mirage Island Spawning");
					_G.MirageSpawn = true;
				else
